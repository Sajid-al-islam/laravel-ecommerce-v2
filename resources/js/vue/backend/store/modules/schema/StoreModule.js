        ...
        const mutations = {
            ...
            * */
            ...
            [`set_selected_${store_prefix}s`]: function (state, data) {
                ...
                if (check_index >= 0) {
                    let el = document.querySelector(`table input[data-id="${data.id}"]`)
                    if(el)el.checked = false;
                    el = document.querySelector(`table input.check_all`)
                    if(el)el.checked = false;
                ...
                if(state[`${store_prefix}_show_management_modal_qty`] == 1 && temp_selected.length){
                    ...
                    [...document.querySelectorAll(`table input[type="checkbox"]`)].forEach(el=>el.checked=false);
